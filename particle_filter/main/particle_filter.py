...
alpha = [.005, .0005, .0005, 0.005]
# map path fi5e
map_file_path = "../data/map/wean.dat"

# log data path
log_path = "../data/log/ascii-robotdata4.log"
...
class ParticleFilter:
    ...
    def __init__(self, map_file, num, alpha, log_file_path, r_step, s_search):
        ...
        # read distance table
        if (os.path.isfile('distance_table.npy')):
            print 'Distance table already there. Using the current table'
        else:
            print 'Creating Distance table as it is not already there'
            self.visuvalize.distance_table.createDistacleTable()
            print 'Done Creating distance table'
